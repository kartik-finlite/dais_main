...
$pageTitle = "Upcomming Event List";
...
<!-- LOCAL PAGE LAVEL JS -->
<script src="assets/web/local_assets/js/event_list_upcoming.js"></script>
            ...
            <div class="card-header">
              <div class="row">
                <div class="col-6 col-md-6 col-xs-6">
                  <h3 class="card-title mt-2">Upcomming Events</h3>
                </div>
                <?php
              ...
              <table class="table table-bordered table-striped datatable">
                ...
                <tbody>
                  ...
                  if ($user_data['user_type_name'] == "Administrator" || $user_data['user_type_name'] == "Core Captain") {
                    ...
                    // LEFT JOIN age_gender_categories agc ON agc.id=eagc.age_gender_id 
                    // LEFT JOIN event_grade_categories egc ON e.id = egc.event_id
                    // LEFT JOIN grades g ON g.id = egc.grade_id WHERE event_date_time > "'.$current_date_time.'" AND EXTRACT(YEAR FROM "'.$user_details['dob'].'") >= agc.start_year and EXTRACT(YEAR FROM "'.$user_details['dob'].'") <= agc.end_year AND agc.gender = "'.$user_details['gender'].'" AND e.title LIKE "%'.$requestField['search'].'%"';
                    // $count_result = mysqli_query($conn, $count_sql);
                    // $count_data = mysqli_fetch_assoc($count_result);



                    // $sql_query = 'SELECT e.*,es.status as is_signup,es.reject_by,es.approved_by,g.start_grade,g.end_grade FROM events e
                    // LEFT JOIN event_age_gender_categories eagc ON eagc.event_id = e.id
                    // LEFT JOIN event_signups es ON e.id = es.event_id and es.user_id = "'.$_SESSION['user_details']['id'].'" 
                    // LEFT JOIN age_gender_categories agc ON agc.id=eagc.age_gender_id 
                    // LEFT JOIN event_grade_categories egc ON e.id = egc.event_id
                    // LEFT JOIN grades g ON g.id = egc.grade_id WHERE event_date_time > "'.$current_date_time.'" OR EXTRACT(YEAR FROM "'.$user_details['dob'].'") >= agc.start_year and EXTRACT(YEAR FROM "'.$user_details['dob'].'") <= agc.end_year AND agc.gender = "'.$user_details['gender'].'" GROUP BY e.id';
                  ...
                  ?>
                    ...
                    <tr style="<?php echo $style; ?>">
                      ...
                      ?>
                        ...
                        if ($row['is_delete'] == 0) {
                        ?>
                          <td><a href="event.php?id=<?php echo MD5($row['id']); ?>"><i class="fas fa-edit" aria-hidden="true"></i></a>&nbsp&nbsp<a href="javascript:void(0)" data-id="<?php echo MD5($row['id']); ?>" class="delete_event"><i class="fas fa-trash"></i></a></td>
                        <?php } else { ?>
                          <td><a href="event.php?id=<?php echo MD5($row['id']); ?>"><i class="fas fa-edit" aria-hidden="true"></i></a></td>
                        <?php } ?>
                      <?php } ?>
                    </tr>
                  <?php } ?>
