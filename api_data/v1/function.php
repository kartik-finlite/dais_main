...
function check_api_token($token = null){
    global $conn;
    $sql = 'SELECT * FROM `users`WHERE api_access_token = "'.$token.'"';
    $result = mysqli_query($conn, $sql);
    $token_data = mysqli_fetch_assoc($result);
    if(isset($token_data) && $token_data > 0){
...
function limit_event_check($user_id = "",$event_id = ""){
	...
	$check_event_query = 'SELECT category_id FROM `events` WHERE id = "'.$event_id.'"';
